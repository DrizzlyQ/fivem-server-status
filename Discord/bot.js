...
const USER_AGENT = `Drizzly Bot ${require('./package.json').version} , Node ${process.version} (${process.platform}${process.arch})`;
...
exports.start = function(SETUP) {
  ...
  const MAX_PLAYERS = 300; //CHANGE THIS DUE TO YOUR PLAYER LIMIT OF YOUR SERVER. THIS IS SERIOUS.
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'SERVER' : 'NAME';
    let embed = new Discord.RichEmbed()
    .setAuthor("Server Status", "LOGO LINK GOES HERE")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ ONLINE' : '⚫ OFFLINE')
    .setTimestamp(new Date())
    .addField('\n\u200b\nsample text)\n\u200b\n',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: server status:',`${STATUS}\n\u200b\n`);
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('SERVER', 'https://www.twitch.tv/drizzlyq');
    bot.user.setActivity('STREAM',{'url':'https://www.twitch.tv/DrizzlyQ','type':'STREAMING'});
